...
冒頭の「今日やること」は最大3件とし、内部IDだけでなく投稿・企画のタイトルと必要な判断を表示する。企画PRがマージ済みでも企画台帳が`オーナー判断待ち`なら、「マージ済み・オーナー判断待ち」と表示し、不整合や承認済みとは扱わない。同じエラーが前日から変化せず、`retryAfter`または次回確認日時が未来なら、優先事項とalertsへ繰り返し載せず、各セクションに状態と次回確認日時を1行だけ表示する。
